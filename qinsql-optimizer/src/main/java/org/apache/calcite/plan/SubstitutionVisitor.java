...
public class SubstitutionVisitor {
    ...
    protected static final ImmutableList<UnifyRule> DEFAULT_RULES = ImmutableList.of(TrivialRule.INSTANCE,
            ScanToProjectUnifyRule.INSTANCE, ProjectToProjectUnifyRule.INSTANCE, FilterToProjectUnifyRule.INSTANCE,
    ...
    public static MutableRel unifyAggregates(MutableAggregate query, MutableAggregate target) {
        ...
        if (query.groupSet.equals(target.groupSet)) {
            ...
            for (AggregateCall aggregateCall : query.aggCalls) {
                ...
                aggregateCalls
                        .add(AggregateCall.create(getRollup(aggregateCall.getAggregation()), aggregateCall.isDistinct(),
                                aggregateCall.isApproximate(), ImmutableList.of(target.groupSet.cardinality() + i), -1,
    ...
    protected static RexShuttle getRexShuttle(MutableProject target) {
        ...
        return new RexShuttle() {
            ...
            public RexNode visitInputRef(RexInputRef ref) {
                ...
                if (integer != null) {
                    return RexBuilder.getRexFactory().makeInputRef(integer, ref.getType());
                }
                throw MatchFailed.INSTANCE;
            }
            ...
            public RexNode visitCall(RexCall call) {
                ...
                if (integer != null) {
                    return RexBuilder.getRexFactory().makeInputRef(integer, call.getType());
                }
    ...
    public static class FilterOnProjectRule extends RelOptRule {
        ...
        public FilterOnProjectRule(RelBuilderFactory relBuilderFactory) {
            super(operandJ(LogicalFilter.class, null, filter -> filter.getCondition() instanceof RexInputRef,
                    some(operand(LogicalProject.class, any()))), relBuilderFactory, null);
        }
