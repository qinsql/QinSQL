...
public class QinResult implements Result { 
    
    // SqlAccessor直接访问ValueVector
    static class QinSqlAccessor {
        final SqlAccessor accessor;
        final int valueType;

        public QinSqlAccessor(SqlAccessor accessor, int valueType) {
    ...
    static class QinAccessor {
        final Accessor accessor;
        final int valueType;

        public QinAccessor(Accessor accessor, int valueType) {
            this.accessor = accessor;
            this.valueType = valueType;
        }
    ...
    int rowCount;
    int index;
    QinAccessor[] accessors;
    Value[] current;
    ArrayList<Value[]> rows = new ArrayList<>();

    public QinResult(RecordBatch data) {
        // this.data = data;
        schema = data.getSchema();
        columnCount = schema.getFieldCount();
        accessors = new QinAccessor[columnCount];
        int column = 0;
        for (VectorWrapper<?> vw : data) {
            ...
            accessors[column++] = new QinAccessor(acc, valueType);
        }
