...
public class Drillbit implements AutoCloseable {
    ...
    public Drillbit(final DrillConfig config, final RemoteServiceSet serviceSet) throws Exception {
        this(config, SystemOptionManager.createDefaultOptionDefinitions(), serviceSet,
                ClassPathScanner.fromPrescan(config));
    }

    @VisibleForTesting
    public Drillbit(final DrillConfig config, final CaseInsensitiveMap<OptionDefinition> definitions,
            final RemoteServiceSet serviceSet) throws Exception {
        this(config, definitions, serviceSet, ClassPathScanner.fromPrescan(config));
    }

    public Drillbit(final DrillConfig config, final RemoteServiceSet serviceSet,
    ...
    @VisibleForTesting
    public Drillbit(final DrillConfig config, final CaseInsensitiveMap<OptionDefinition> definitions,
            final RemoteServiceSet serviceSet, final ScanResult classpathScan) throws Exception {
        this.config = config;
    ...
    public void run() throws Exception {
        ...
        manager.start(md, engine.getController(), engine.getDataConnectionCreator(), coord,
    ...
    public synchronized void close() {
        ...
        try {
            AutoCloseables.close(engine, storeProvider, coord, manager, storageRegistry, context);
    ...
    private static class GracefulShutdownThread extends Thread {
        ...
        public void run() {
            try {
                ...
                throw new RuntimeException(
                        "Caught exception while polling for gracefulshutdown\n" + stackTrace, e);
            }
        }
        ...
        private void pollShutdown(Drillbit drillbit) throws IOException, InterruptedException {
            ...
            try (final WatchService watchService = drillHomePath.getFileSystem().newWatchService()) {
                drillHomePath.register(watchService, StandardWatchEventKinds.ENTRY_MODIFY,
