 ...
 */
// 改编自org.h2.server.TcpServerThread
public class BatsServerThread implements Runnable {
    ...
    private final SmallLRUCache<Long, CachedInputStream> lobs = SmallLRUCache
            .newInstance(Math.max(SysProperties.SERVER_CACHED_OBJECTS, SysProperties.SERVER_RESULT_SET_FETCH_SIZE * 5));
            ...
            try {
                ...
                if (maxClientVersion < Constants.TCP_PROTOCOL_VERSION_MIN_SUPPORTED) {
                    throw DbException.get(ErrorCode.DRIVER_VERSION_ERROR_2, "" + clientVersion,
                            "" + Constants.TCP_PROTOCOL_VERSION_MIN_SUPPORTED);
    ...
    private void process() throws IOException {
        ...
        case SessionRemote.COMMAND_EXECUTE_UPDATE: {
            ...
            if (supportsGeneratedKeys) {
                ...
                default:
                    throw DbException.get(ErrorCode.CONNECTION_BROKEN_1, "Unsupported generated keys' mode " + mode);
                }
            } else {
                generatedKeysRequest = false;
            }
            ...
            transfer.writeInt(status).writeInt(result.getUpdateCount()).writeBoolean(session.getAutoCommit());
        ...
        case SessionRemote.SESSION_HAS_PENDING_TRANSACTION: {
            transfer.writeInt(SessionRemote.STATUS_OK).writeInt(session.hasPendingTransaction() ? 1 : 0).flush();
            break;
        }
