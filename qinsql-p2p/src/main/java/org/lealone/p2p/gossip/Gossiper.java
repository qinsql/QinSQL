...
public class Gossiper implements IFailureDetectionEventListener, GossiperMBean {
            ...
            try {
                ...
                if (logger.isTraceEnabled())
                    logger.trace("My heartbeat is now {}",
                            nodeStateMap.get(ConfigDescriptor.getLocalNode()).getHeartBeatState()
                                    .getHeartBeatVersion());
                ...
                if (gDigests.size() > 0) {
                    GossipDigestSyn digestSynMessage = new GossipDigestSyn(
                            ConfigDescriptor.getClusterName(), gDigests);
    ...
    public void start(int generationNbr, Map<ApplicationState, VersionedValue> preloadLocalStates) {
        ...
        ScheduledExecutors.scheduledTasks.scheduleWithFixedDelay(new GossipTask(),
        ...
        if (size > 0) {
            ...
            if (liveNodes.isEmpty()) {
                ...
                double probability = seeds.size()
                        / (double) (liveNodes.size() + unreachableNodes.size());
                double randDbl = random.nextDouble();
            ...
            if (epState != null) {
                ...
                if (isGossipOnlyMember(node) && !justRemovedNodes.containsKey(node)
                        && TimeUnit.NANOSECONDS
                                .toMillis(nowNano - epState.getUpdateTimestamp()) > FAT_CLIENT_TIMEOUT) {
        ...
        if (!justRemovedNodes.isEmpty()) {
            for (Entry<NetNode, Long> entry : justRemovedNodes.entrySet()) {
                if ((now - entry.getValue()) > QUARANTINE_DELAY) {
                    if (logger.isDebugEnabled())
                        logger.debug("{} elapsed, {} gossip quarantine over", QUARANTINE_DELAY,
                                entry.getKey());
    ...
    public void advertiseRemoving(NetNode node, UUID hostId, UUID localHostId) {
        ...
        epState.addApplicationState(ApplicationState.STATUS,
                P2pServer.valueFactory.removingNonlocal(hostId));
    ...
    public void assassinateNode(String address) throws UnknownHostException {
        ...
        if (epState == null) {
            epState = new NodeState(
                    new HeartBeatState((int) ((System.currentTimeMillis() + 60000) / 1000), 9999));
        } else {
            int generation = epState.getHeartBeatState().getGeneration();
            ...
            if (newState == null)
                ...
                throw new RuntimeException("Node still alive: " + node
        ...
        epState.addApplicationState(ApplicationState.STATUS,
                P2pServer.valueFactory.left(null, computeExpireTime()));
    ...
    public NetNode getTcpNode(NetNode node) {
        if (!usesHostId(node))
            throw new RuntimeException("Host " + node + " does not use new-style tokens!");
        return NetNode
                .createP2P(getNodeState(node).getApplicationState(ApplicationState.TCP_NODE).value);
    }
    ...
    public NodeState getStateForVersionBiggerThan(NetNode forNode, int version) {
        ...
        if (epState != null) {
            ...
            */
            ...
            for (Entry<ApplicationState, VersionedValue> entry : epState.getApplicationStateMap()
                    .entrySet()) {
            ...
            */
            if (localEpStatePtr != null) {
                ...
                if (localGeneration != 0
                        && remoteGeneration > localGeneration + MAX_GENERATION_DIFFERENCE) {
                    ...
                    if (logger.isTraceEnabled())
                        logger.trace("Updating heartbeat state generation to {} from {} for {}",
                                remoteGeneration, localGeneration, ep);
                ...
                } else if (remoteGeneration == localGeneration) // generation has not changed, apply new states
                {
                    ...
                    if (remoteMaxVersion > localMaxVersion) {
                        ...
                        logger.trace("Ignoring remote version {} <= {} for {}", remoteMaxVersion,
                                localMaxVersion, ep);
    ...
    private void applyNewStates(NetNode addr, NodeState localState, NodeState remoteState) {
        ...
        for (Entry<ApplicationState, VersionedValue> remoteEntry : remoteState.getApplicationStateMap()
                .entrySet()) {
            ApplicationState remoteKey = remoteEntry.getKey();
            VersionedValue remoteValue = remoteEntry.getValue();

            assert remoteState.getHeartBeatState().getGeneration() == localState.getHeartBeatState()
                    .getGeneration();
    ...
    private void requestAll(GossipDigest gDigest, List<GossipDigest> deltaGossipDigestList,
    ...
    private void sendAll(GossipDigest gDigest, Map<NetNode, NodeState> deltaEpStateMap,
    ...
    */
                    ...
                    */
                    if (maxRemoteVersion > maxLocalVersion) {
                        deltaGossipDigestList.add(
                                new GossipDigest(gDigest.getNode(), remoteGeneration, maxLocalVersion));
    ...
    public void doShadowRound() {
        ...
        GossipDigestSyn digestSynMessage = new GossipDigestSyn(ConfigDescriptor.getClusterName(),
                gDigests);
        P2pPacketOut<GossipDigestSyn> message = new P2pPacketOut<>(digestSynMessage);
        inShadowRound = true;
